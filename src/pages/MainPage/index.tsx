...
export default function () {
  ...
  useEffect(() => {
    getScenes();
    const isLocalPreview = window.location.host.startsWith('localhost');
    const handler = () => {
      if (
    ...
    !isLocalPreview && document.addEventListener('visibilitychange', handler);
    return () => {
      !isLocalPreview && document.removeEventListener('visibilitychange', handler);
    };
  }, []);
